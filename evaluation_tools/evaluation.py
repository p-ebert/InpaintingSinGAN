...
#%%
images = ["127h.png","hike.png","image_3.png", "image_4.png","image_6.png","image_8.png", "mountain.png","road.png","stairway.png","tree.png","village.png"]

for i in images:
    ground_truth = cv2.imread("Evaluation/true/%s" % (i))
    singan = cv2.imread("Evaluation/singan/%s" % (i))
    telea30 = cv2.imread("Evaluation/telea/%s" % (i))
    nvidia = cv2.imread("Evaluation/nvidia/%s" % (i))

    print(i, ground_truth.shape)
    print(i, singan.shape)
    print(i, telea30.shape)
    print(i, nvidia.shape)
#%%
#images
nvidia = cv2.imread("Evaluation/nvidia/image_4.png")
singan = cv2.imread("Evaluation/singan/image_4.png")
true = cv2.imread("Evaluation/image_4M.png")

fig, ax  = plt.subplots(1,3 ,figsize=(15,15))
fig.tight_layout(h_pad=-70)
#ax[0].set_title("Original image")
ax[0].set_axis_off()
ax[0].imshow(cv2.cvtColor(true, cv2.COLOR_BGR2RGB))

#ax[1].set_title("Masked image")
ax[1].set_axis_off()
ax[1].imshow(cv2.cvtColor(singan, cv2.COLOR_BGR2RGB))

ax[2].set_axis_off()
ax[2].imshow(cv2.cvtColor(nvidia, cv2.COLOR_BGR2RGB))
fig.savefig("Evaluation/image_4_comp.png")
